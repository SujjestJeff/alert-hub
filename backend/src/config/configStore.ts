...
class ConfigStore extends EventEmitter {
  ...
  init(): void {
    const seedAlert = db.prepare(
      `INSERT OR IGNORE INTO alert_config (kind, data) VALUES (?, ?)`,
    );
    const readAlert = db.prepare(
      `SELECT data FROM alert_config WHERE kind = ?`,
    );
    ...
    const s = db.prepare(`SELECT data FROM settings WHERE id = 1`).get() as {
      data: string;
    };
  ...
  getAll() {
    return { alerts: Object.fromEntries(this.alerts), settings: this.settings };
  }
  getAlert(kind: AlertKind) {
    return this.alerts.get(kind);
  }
  getSettings() {
  ...
  updateAlert(kind: AlertKind, patch: unknown): AlertConfig {
    ...
    db.prepare(`UPDATE alert_config SET data = ? WHERE kind = ?`).run(
      JSON.stringify(next),
      kind,
    );
