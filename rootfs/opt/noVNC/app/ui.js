...
const UI = {
  ...
  async start() {
    ...
    Array.from(document.getElementsByName('noVNC_app_name')).forEach(
      (el) => (el.innerText = UI.webData.applicationName),
    );

    // Update logo image properties.
    document.getElementById('noVNC_app_logo').alt =
      UI.webData.applicationName + 'logo';
    document.getElementById('noVNC_app_logo').title =
      UI.webData.applicationName;

    // Set or hide the application version.
    if (UI.webData.applicationVersion) {
      document.getElementById('noVNC_version_app').innerText =
        UI.webData.applicationName + ' v' + UI.webData.applicationVersion;
      document
        .getElementById('noVNC_version_app')
        .classList.remove('noVNC_hidden');
    }

    // Set or hide the Docker image version.
    if (UI.webData.dockerImageVersion) {
      document.getElementById('noVNC_version_docker_image').innerText =
        'Docker Image v' + UI.webData.dockerImageVersion;
      document
        .getElementById('noVNC_version_docker_image')
        .classList.remove('noVNC_hidden');
    }
    ...
    if (UI.webData.applicationVersion || UI.webData.dockerImageVersion) {
      document
        .getElementById('noVNC_version_footer')
        .classList.remove('noVNC_hidden');
    }
    ...
    if (!(window.AudioContext || window.webkitAudioContext)) {
      ...
      UI.updateLogging();
      document
        .getElementById('noVNC_audio_section')
        .classList.remove('noVNC_hidden');
      ...
      document.getElementById('noVNC_setting_audio_volume').value =
        volume.toString();
    }
    ...
    if (UI.webData.webAuthSupport) {
      document
        .getElementById('noVNC_logout_button')
        .classList.remove('noVNC_hidden');
    }
    ...
    if (UI.webData.fileManager) {
      ...
      document
        .getElementById('noVNC_file_manager_button')
        .classList.remove('noVNC_hidden');
    ...
    if (UI.webData.terminal) {
      ...
      document
        .getElementById('noVNC_terminal_button')
        .classList.remove('noVNC_hidden');
    ...
    if (window.innerHeight < 600) {
      document.getElementById('settingsCollapse').classList.remove('show');
      document.getElementById('clipboardCollapse').classList.remove('show');
    }
  ...
  initSettings() {
    ...
    if (isTouchDevice) {
      ...
      UI.webData.applicationWindowWidth &&
      window.innerWidth < 0.8 * UI.webData.applicationWindowWidth
    ) {
      // ... unless the browser's window width is less than 80% of the
      // defined application's window width.
      resize = 'scale';
    } else if (
      UI.webData.applicationWindowHeight &&
      window.innerHeight < 0.8 * UI.webData.applicationWindowHeight
    ) {
  ...
  fetchWebData() {
    return fetch('./webdata.json')
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `Could not fetch web data: HTTP error: Status: ${response.status}`,
          );
        }
  ...
  addJumpRoomHandlers() {
    let vncContainer = document.getElementById('noVNC_container');
    let canvasElm = vncContainer.querySelector('canvas');
    const urlParams = new URLSearchParams(window.location.search);
    const host_url = urlParams.get('jr_host_path');
    const host_protocol = urlParams.get('protocol');

    canvasElm.addEventListener('mousemove', function (e) {
      const coords = { clientX: e.clientX, clientY: e.clientY };
      try {
        window.parent.postMessage(coords, host_protocol + '//' + host_url);
      } catch (_) {}
    });

    canvasElm.addEventListener('keydown', function (e) {
      var isMac = navigator.platform.toUpperCase().indexOf('MAC') >= 0;

      if (
        (isMac && e.key === 'v' && e.metaKey) ||
        (!isMac && e.key === 'v' && e.ctrlKey)
      ) {
              ...
              if (mimeType === 'text/plain') {
                item.getType('text/plain').then((blob) => {
                  blob.text().then((blobText) => {
                    UI.rfb.clipboardPasteFrom(blobText);
                    UI.rfb.sendKey(65513, 'MetaLeft', false);
                    UI.rfb.sendKey(65507, 'ControlLeft', true);
                    UI.rfb.sendKey(118, 'KeyV', true);
                    UI.rfb.sendKey(118, 'KeyV', false);
                    UI.rfb.sendKey(65507, 'ControlLeft', false);
                  });
                });
                break;
              }
      ...
      if (
        (isMac && e.key === 'c' && e.metaKey) ||
        (!isMac && e.key === 'c' && e.ctrlKey)
      ) {
        // Send keystrokes to copy into vnc clipboard
        UI.rfb.sendKey(65513, 'MetaLeft', false);
        UI.rfb.sendKey(65507, 'ControlLeft', true);
        UI.rfb.sendKey(99, 'KeyC', true);
        UI.rfb.sendKey(99, 'KeyC', false);
        UI.rfb.sendKey(65507, 'ControlLeft', false);
      }
    });
  },
  ...
  addAudioHandlers() {
    document
      .getElementById('noVNC_audio_button')
  ...
  openControlbar() {
    ...
    if (
      !isTouchDevice &&
      document.getElementById('clipboardCollapse').classList.contains('show')
    ) {
  ...
  updateActionIconsSection(mutationsList, observer) {
    ...
    // class.
    if (
      !document.documentElement.classList.contains('noVNC_touch') &&
      document
        .getElementById('noVNC_fullscreen_button')
        .classList.contains('noVNC_hidden') &&
      document
        .getElementById('noVNC_view_drag_button')
        .classList.contains('noVNC_hidden')
      ...
      document
        .getElementById('noVNC_action_icons_section')
        .classList.add('noVNC_hidden');
    } else {
      document
        .getElementById('noVNC_action_icons_section')
  ...
  securityFailed(e) {
    ...
    if ('reason' in e.detail) {
      msg = 'New connection has been rejected: ' + e.detail.reason;
    } else {
  ...
  updateClipboard() {
    ...
    if (UI.rfb.clipboardAutoSync === 'granted') {
      ...
      document
        .getElementById('noVNC_clipboard_section')
        .classList.add('noVNC_hidden');
    } else {
      UI.addClipboardHandlers();
      UI.rfb.addEventListener('clipboard', UI.clipboardReceive);
  ...
  toggleAudio() {
    ...
    WebUtil.writeSetting(
      'audio_enabled',
      UI.audioContext.audioEnabled ? '1' : '0',
    );
    ...
    if (UI.audioContext.audioEnabled) {
      ...
      document.getElementById('noVNC_setting_audio_volume').value = Math.max(
        15,
        volume,
      ).toString();
    } else {
  ...
  updateAudio() {
    ...
    function connectWebSocket() {
      ...
      UI.audioContext.webSocket.onclose = function (event) {
        if (event.wasClean) {
          Log.Info(
            `WebSocket connection for audio closed, code=${event.code} reason=${event.reason}`,
          );
        } else {
