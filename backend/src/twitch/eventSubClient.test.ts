...
const fakeSocket = () =>
  ({ close: vi.fn(), terminate: vi.fn(), on: vi.fn() }) as any;
const now = () => new Date().toISOString();

const welcome = (id = 'sess1') =>
  JSON.stringify({
    metadata: {
      message_id: `w-${id}`,
    ...
    payload: {
      session: { id, keepalive_timeout_seconds: 10, reconnect_url: null },
    },
  });
const notification = (type = 'channel.follow', id = 'n1') =>
  JSON.stringify({
    metadata: {
      message_id: id,
    ...
    payload: {
      subscription: { id: 's', type, status: 'enabled' },
      event: { user_name: 'Alice' },
    },
  });
const reconnect = (url = 'wss://new/ws') =>
  JSON.stringify({
    metadata: {
      message_id: 'r1',
...
describe('the resubscribe distinction', () => {
  it('subscribes on a FRESH welcome', async () => {
    ...
    expect(
      (createSubscription as any).mock.calls.length,
