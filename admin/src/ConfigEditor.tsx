    ...
    <div className="editor">
      ...
      {KINDS.map((k) => (
        <AlertCard
          ...
          onSaved={(next: any) =>
            onChange({ ...config, alerts: { ...config.alerts, [k]: next } })
          }
...
function AlertCard({
  ...
  return (
    <section className="card">
      ...
      <button onClick={() => previewRef.current?.previewDraft(kind, draft)}>
        Preview (draft)
      </button>
      <button onClick={() => fireTestAlert(kind)}>
        Fire test (live to OBS)
      </button>
