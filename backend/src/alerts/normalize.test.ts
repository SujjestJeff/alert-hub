...
describe('normalize', () => {
  it('maps a follow', () => {
    expect(
      normalize(notif('channel.follow', { user_name: 'Alice' })),
  ...
  it('maps a gift with its total and tier', () => {
    expect(
      normalize(
        notif('channel.subscription.gift', {
          user_name: 'Carol',
          total: 20,
          tier: '1000',
        }),
  ...
  it('reads the resub message from the object shape', () => {
    expect(
      normalize(
        notif('channel.subscription.message', {
          user_name: 'Dee',
          tier: '3000',
  ...
  it('reads the cheer message from the string shape', () => {
    expect(
      normalize(
        notif('channel.cheer', { user_name: 'Eve', bits: 500, message: 'pog' }),
      ),
    ).toMatchObject({
      kind: 'cheer',
      displayName: 'Eve',
