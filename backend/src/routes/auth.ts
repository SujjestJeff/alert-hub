...
export default async function authRoutes(app: FastifyInstance) {
  ...
  app.get('/auth/callback', async (req, reply) => {
    ...
    if (error)
      return reply
        .code(400)
        .send(`Twitch denied authorization: ${error_description ?? error}`);
    if (!code || !state) return reply.code(400).send('Missing code or state.');
    if (!consumeState(state))
      return reply
        .code(400)
        .send('Invalid or expired state. Start again at /auth/login.');
    ...
    try {
      ...
      return reply
        .code(502)
        .send('Could not complete authorization with Twitch.');
    }
  });
  ...
  app.get('/auth/success', async (_req, reply) =>
    reply
      .type('text/html')
      .send('<h1>Connected to Twitch</h1><p>You can close this tab.</p>'),
  );
}
